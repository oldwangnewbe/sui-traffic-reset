...
CHECK_INTERVAL=60
TZ=Asia/Shanghai
RESET_WEB_TOKEN=please-change-this-token
...
| `TZ` | 容器时区 |
| `RESET_WEB_TOKEN` | Web 控制台和 API 的访问 Token |
...
- 如果公网开放 Web 页面，请使用强 Token，并建议套反代 HTTPS。
...
自动重置规则存放在这张表中，不会修改 s-ui 自身表结构。执行重置时会更新：
